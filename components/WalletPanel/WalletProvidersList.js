...
const dappUrl = "nft.chaintusker.com/";
            ...
            <ul className="divide-y-2 divide-gray-100"> {
                Object.values(connectors).map((connector) => {
                    ...
                    if (isMobileDevice() && name === "Metamask" && !window.ethereum) {
                        return (<li className="shadow-around cursor-pointer"
                            key={name}>
                            <a href={metamaskAppDeepLink}>
                                <button className="p-3 w-full flex items-center font-bold" type="button">
                    ...
                    return (<li className="shadow-around cursor-pointer"
                        ...
                        <button className="p-3 w-full flex items-center font-bold" type="button"
                            onClick={
                                () => handleToggleConnect(method, name)
                        }>
                            <div className="h-full w-6 mr-4 relative flex"> {logo}</div>
                            {name} </button>
                    </li>);
